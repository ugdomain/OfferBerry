...
class HomePageFields {
  ...
  void saveDataToExcel() {
    ...
    _excelController
        .addDataToJsonList({..._attrController.getJsonControlValues()});
    _excelController
        .addItemsTomapOfItems({..._attrController.getControlValues()});
  }

  void saveData() {
    bool isValid = _attrController.key.currentState!.validate();
    if (isValid && _attrController.validate.value) {
      print("radioButton value = ${_attrController.radioButtonMap}");
...
class BuildInputFields extends StatelessWidget {
  ...
  final SubCategoryAttrController _attrController = Get.find();
  final StaticFieldsController _fieldsController = Get.find();
  final ChooseImageController imageCon = Get.find();
    ...
    return Container(
      ...
      child: Obx(
        () => _attrController.attrList.isNotEmpty
            ? TextFormField(
                initialValue: value,
                keyboardType: _attrController.attrList[0].attrs![index].type,
                key: key,
                decoration: InputDecoration(
                    hintText:
                ...
                onSaved: (value) {
                  ...
                  if (isValidated) {
                    _attrController.setInputValue(
                        ...
                        value);
                    _fieldsController.addDataToJsonMap({
                      _attrController.attrList.first.attrs![index].jsonName:
                          value
                    });
                  }
                },
