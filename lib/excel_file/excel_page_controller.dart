...
class ExcelController extends GetxController {
  ...
  Future<void> createExcel() async {
    ...
    map.forEach((k, v) {
      ...
      if (mapOfItems.containsKey(k) ||
          mapOfItems.containsKey(k.replaceAll("\"", ""))) {
        DataValidation cell =
            sheet.getRangeByIndex(2, col, 1000, col).dataValidation;
    ...
    try {
      for (var table in excel.tables.keys) {
        ...
        for (var row in excel.tables[table]!.rows) {
          ...
          var catIdAndSubId = {
            Variables.categoryName[1]: _categoryController.categoryId.value,
            Variables.subCategoryName[1]:
          ...
          for (var i in list) {
            if (list.contains(Variables.name[0])) {
              ...
              } else if (i == Variables.unitPrice[0]) {
                jsonKeys.add(Variables.unitPrice[1]);
              } else if (i == Variables.description[0]) {
  ...
  Future<void> uploadExcel() async {
    ...
    mapOfExcelData.then((value) {
      ...
      if (isListValid) {
        ...
        Get.snackbar("Success", "Data Upload Failed!",
            snackPosition: SnackPosition.BOTTOM);
      }
    });
  }
